...
def test_node_index_creation(client):
    ...
    res = graph.create_node_vector_index(lbl, 'desc', dim=32, similarity_function="euclidean")
    assert(res.indices_created == 1)
...
def test_edge_index_creation(client):
    ...
    res = graph.create_edge_vector_index(rel, 'desc', dim=32, similarity_function="euclidean")
    assert(res.indices_created == 1)
