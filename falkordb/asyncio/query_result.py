...
async def __parse_entity_properties(props, graph):
    ...
    properties = {}
    for prop in props:
        prop_name = await graph.schema.get_property(prop[0])
        prop_value = await parse_scalar(prop[1:], graph)
...
async def __parse_node(value, graph) -> Node:
    ...
    labels = None
    if len(value[1]) > 0:
        labels = [await graph.schema.get_label(inner_label) for inner_label in value[1]]
...
async def __parse_edge(value, graph) -> Edge:
    ...
    """
    edge_id = int(value[0])
    relation = await graph.schema.get_relation(value[1])
...
async def __parse_map(value, graph) -> OrderedDict:
    ...
    for i in range(0, n_entries, 2):
        key = await __parse_string(value[i], graph)
        m[key] = await parse_scalar(value[i + 1], graph)
...
class QueryResult:
    ...
    async def parse(self, response):
        ...
        if len(response) == 1:
            ...
            await self.__parse_results(response)
    ...
    async def __parse_results(self, raw_result_set):
        """
        ...
        self.result_set = await self.__parse_records(raw_result_set)
    ...
    async def __parse_records(self, raw_result_set):
        """
